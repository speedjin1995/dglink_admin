...
if(isset($post['staffName'], $post['customer'])){
	...
	if(isset($post['userId']) && $post['userId'] != null && $post['userId'] != ''){
	    if ($update_stmt = $db->prepare("UPDATE products SET product_name = ? WHERE id = ?")) {
	    ...
	    if ($insert_stmt = $db->prepare("INSERT INTO products (product_name, customer) VALUES (?, ?)")){	
